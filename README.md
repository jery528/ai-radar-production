...
**数据库驱动 + 页面全要素后台可配置 + GLM-5.2 智能分类/摘要/洞察日报**。
...
- **GLM-5.2 三能力**：关键词判不准的条目智能分类、热门英文条目中文摘要、每轮抓取后生成「AI 机会洞察日报」；支持每轮上限、每日 token 预算、熔断降级（大模型故障不影响抓取）
...
| 大模型 | 总开关 / API 地址 / Key / 模型 / 分类·摘要·日报独立开关与上限 / 每日 token 预算 | glm-5.2，预算 50 万 |
| 站点 | 浏览器标题 / SEO 描述 | — |
...
- GLM-5.2 是思考型模型，本项目默认关闭 thinking 以节省 token；日报生成约 10–30 秒
